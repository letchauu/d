<?php
    ...
    if(isset($_POST['editarNoticia'])) {
        ...
        if(isset($_FILES['fotoNoticia']) && $_FILES['fotoNoticia']['error'] == UPLOAD_ERR_OK) {
            ...
            if(move_uploaded_file($fotoNoticia['tmp_name'], $caminhoNovaFoto)) {
                ...
                if(mysqli_num_rows($queryFotoAntiga) > 0) {
                    ...
                    $caminhoNovaAntiga = $dadosNoticia['fotoNoticia'];
                    if(file_exists($caminhoNovaAntiga) && is_file($caminhoNovaAntiga)) {
                        unlink($caminhoNovaAntiga);
                    }
                }
            } else {
        ...
        if($atualizarFoto) {
            $sql = "UPDATE noticias SET tituloNoticia = '$tituloNoticia', textoNoticia = '$textoNoticia', fotoNoticia = '$caminhoNovaFoto' 
            WHERE idNoticia = '$noticia_id'";
        } else {
